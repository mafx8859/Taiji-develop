            ...
            <table id="example" class="table display table-bordered">
                ...
                <c:forEach items="${activityList}" var="activity" varStatus="status">
                    <tr>
                        ...
                        <td>
                            <a href="${pageContext.request.contextPath}/activity/detail/${activity.id}" class="btn btn-default">查看活动详情</a>
                            <c:choose>
                                <c:when test="${activity.isFinish == -1}">
                                <a href="javascript:;" class="btn btn-danger" onclick="alert('此活动已结束，无法报名！')">此活动已结束</a>
                            </c:when>
                                <c:when test="${activity.isFinish != -1}">
                                    <a href="${pageContext.request.contextPath}/application/organization/${activity.id}" class="btn btn-success">统一报名</a>
                                </c:when>
                            </c:choose>
                        </td>
                    </tr>
